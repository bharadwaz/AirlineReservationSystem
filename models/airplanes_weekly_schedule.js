...
function save_one(fields,cb){
	var insertString = modelHelper.getInsertStatement("AIRPLANES_WEEKLY_SCHEDULE",fields);
...
function save_bulk(arr_fields,cb,batchSize){
	...
	for(var i = 0 ; i < arr_fields.length; i++){
		insert_stmts.push( modelHelper.getInsertStatement("AIRPLANES_WEEKLY_SCHEDULE",arr_fields[i]) );
	}
...
function fetchByCols(fields,condition,cb,offset,limit){
   ...
   var select_stmt = modelHelper.getSelecStatement("AIRPLANES_WEEKLY_SCHEDULE",fields,null,offset,limit);
